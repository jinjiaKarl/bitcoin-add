...
func b58checkencode1(ver uint8, b []byte) (s string)  {
	/* Prepend version */
	fmt.Println("1 - Add version byte in front of RIPEMD-160 hash (0x00 for Main Network)")
	...
	/* SHA256 Hash #1 */
	fmt.Println("2 - Perform SHA-256 hash on the extended RIPEMD-160 result")
	sha256H.Reset()
	sha256H.Write(bcpy)
	...
	/* SHA256 Hash #2 */
	fmt.Println("3 - Perform SHA-256 hash on the result of the previous SHA-256 hash")
	sha256H.Reset()
	sha256H.Write(hash1)
	...
	fmt.Println("4 - Take the first 4 bytes of the second SHA-256 hash. This is the address checksum")
	fmt.Println(byteString(hash2[0:4]))
	fmt.Println("=======================")

	fmt.Println("5 - Add the 4 checksum bytes from stage 7 at the end of extended RIPEMD-160 hash from stage 4. This is the 25-byte binary Bitcoin Address.")
