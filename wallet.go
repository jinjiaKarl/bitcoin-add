...
const version = byte(0x00)  //生成地址时格式为Ox00
const addressChecksumLen = 4  //使用Base58Check时取4位作为错误校验位
...
const privKeyBytesLen = 32  //32字节

func newKeyPair() ([]byte, []byte) {

	curve := elliptic.P256()
	private, err := ecdsa.GenerateKey(curve, rand.Reader)  //使用ECDSA生成私钥
	if err != nil {
		log.Panic(err)
	}
	d := private.D.Bytes()
